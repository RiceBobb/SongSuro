...
class EuclideanCodebook(Module):
	def __init__(
		...
		self.codebook_mean_needs_init = True
		self.codebook_variance_needs_init = True
	...
	def forward(
		...
		if self.affine_param:
			...
			embed = (self.embed - self.codebook_mean) * (
		...
		if exists(codebook_transform_fn):
			...
			dist = -cdist(flatten.type_as(embed), embed)
