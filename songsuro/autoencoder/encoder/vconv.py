...
class GridRange(object):
	...
	def __init__(self, full, sub, gs):
		if not (
			isinstance(sub[0], int)
			and isinstance(sub[1], int)
			and isinstance(full[0], int)
			and isinstance(full[1], int)
		):
...
class VirtualConv(object):
	...
	def __init__(
		...
		if isinstance(filter_info, tuple):
			...
			raise RuntimeError(
				"filter_info must be either a 2-tuple of "
		...
		if self.l_pad > self.l_wing_sz or self.r_pad > self.r_wing_sz:
			# print(self)
			raise RuntimeError(
				"Filter wing sizes cannot be less than the respective " "padding"
			)
		# print(self)
	...
	def get_index_trim(self):
		...
		if self.input_trim is None:
			...
			return [self.input_trim[0], self.input_gr.sub_length() - self.input_trim[1]]
	...
	def _output_offsets(self):
		...
		if self.l_pad != 0 or self.r_pad != 0 or self.stride != 1:
			raise RuntimeError(
				"Can only call output_offset with no padding and " + "unit stride"
			)
...
def compute_inputs(end_vc, end_gr):
	...
	while True:
		...
		if vc.do_trim_input:
			vc.input_trim = [
				vc.input_gr.sub[0] - gr.sub[0],
				gr.sub[1] - vc.input_gr.sub[1],
			]
		else:
			vc.input_gr = gr
	return gr
