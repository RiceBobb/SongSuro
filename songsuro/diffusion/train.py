...
class DiffWaveLearner:
	def __init__(
		...
		denoiser_dir,
		denoiser,
		conditional_encoder_dir,
		conditional_encoder,
		dataset,
		optimizer,
		sample_rate: int,
		noise_schedule: np.ndarray = np.linspace(1e-4, 0.05, 50),
		max_grad_norm=None,
		*args,
		**kwargs,
	):
		os.makedirs(denoiser_dir, exist_ok=True)
		os.makedirs(conditional_encoder_dir, exist_ok=True)
		self.denoiser_dir = denoiser_dir
		self.denoiser = denoiser
		self.conditional_encoder_dir = conditional_encoder_dir
	...
	def train_step(self, features):
		for param in self.model.parameters():  # 그라디언트 초기화
			param.grad = None
		...
		N, T = audio.shape
		device = audio.device
		self.noise_level = self.noise_level.to(
			device
		)  # beta. 노이즈 스케줄. 특성 텐서임

		with self.autocast:
			t = torch.randint(
				0, len(self.noise_schedule), [N], device=audio.device
			)  # 특정 스텝 샘플링
			noise_scale = self.noise_level[t].unsqueeze(1)  # 차원 축소
	...
	def _write_summary(self, step, features, loss):
		...
		writer.add_audio(
			"feature/audio", features["audio"][0], step, sample_rate=self.sample_rate
		)
		writer.add_image(
