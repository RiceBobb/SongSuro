...
@pytest.mark.parametrize(
	"batch_size, seq_len, input_channel, hidden_channels",
	[
		(2, 16, 100, 192),
		(4, 32, 50, 256),
	],
)
def test_fft_encoder_forward_shape(batch_size, seq_len, input_channel, hidden_channels):
	x = torch.randint(0, input_channel, (batch_size, seq_len), dtype=torch.long)
...
def test_fft_encoder_padding_mask():
	batch_size, seq_len, input_channel, hidden_channels = 2, 10, 50, 64
	x = torch.randint(0, input_channel, (batch_size, seq_len), dtype=torch.long)
...
def test_fft_encoder_gradients():
	batch_size, seq_len, input_channel, hidden_channels = 2, 8, 30, 32
	x = torch.randint(0, input_channel, (batch_size, seq_len), dtype=torch.long)
